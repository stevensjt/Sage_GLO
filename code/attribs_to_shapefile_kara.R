...
d_simple <- full_join(d_1940,d_1880) #joining 1940,1880 
d_simple <- full_join(d_simple,d_2004) #now joining 2004 to those
#fuckyes!!!!!
...
attribs <- left_join(lines,as.data.frame(d_simple)) #now joining metadata to atrbt table
#plot(attribs["Sage_1940"])
#plot(attribs["Sage_1880"])
#NOTHING HAPPENS? on either line
...
for(id in ids){
  ...
  l[[id]] <- full_join(d_simple,d_2004) 
  if(id == ids[1]){
    d_full <- l[[id]]
    lines_full <- lines
  } else {
...
##got as far as this, check loop, only worked for 2004 data
