...
d_2019 <- read_xlsx(paste0("./data/",id,"_data.xlsx"), sheet = "2019Survey") %>%
  filter(Survey_type == "transect_summary") %>%
  select(Segment_id,Sage) %>%
  rename(Sage_2019 = Sage)
d_simple <- full_join(d_1940,d_1880)
d_simple<-full_join(d_simple, d_2019) #add 2019 data
...
for(id in ids){
  ...
  l[[id]] <- full_join(l[[id]],d_2019)#add 2019
...
attribs$Sage_2019_simple <- ifelse(attribs$Sage_1940%in%c("present","understory","dense"),
                                   "present",attribs$Sage_2019)

plot(attribs["Sage_1880_simple"])
plot(attribs["Sage_1940_simple"])
plot(attribs["Sage_2019_simple"])
